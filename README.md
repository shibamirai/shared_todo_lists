...
- Laravel
    ...
    - ビルトイン認証は使用せず(プロジェクト作成時に`Authentication Provider`は`None`に設定)、ログイン・ログアウト処理は自分で実装してください
      その際、ログインユーザー名にはメールアドレスを使用するようにしてください
    - 完了状態切替機能はLivewireを使用して実装してください
    - モックアップではTailwindCSSをCDNで利用していますが、CDNではなくプロジェクト内のTailwindCSSを使用してください（HTMLのheadタグ内に`@vite(['resources/css/app.css', 'resources/js/app.js'])`を追加していると使用できます）
...
デフォルトで用意される users テーブルのマイグレーションファイルを修正し、下記のカラムを追加・変更してください。それに合わせて User モデルも修正し、マイグレーションをやり直し（migrate:refresh）してください。  

|カラム名|データ型|制約|デフォルト|備考|
|---|---|---|---|---|
|family_name|string| | |ユーザー姓<br>nameカラムを変更|
|first_name|string| | |ユーザー名<br>新規追加|
|is_admin|boolean| |false|管理者権限<br>新規追加|
|deleted_at|softDeletes| | |削除日時($table->softDeletes()で作成<br>新規追加|

※ email, email_verified_at, password, rememberToken, timestamps は元のまま使用する
